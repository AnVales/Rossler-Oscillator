...
def num_rossler(x1_n, y1_n, z1_n, x2_n, y2_n, z2_n, h, a, b, c, w1, w2):
    x1_n1=x1_n+h*(-w1*y1_n-z1_n) # dx/dt = x1 + h*( - y1 - z1)
    y1_n1=y1_n+h*(w1*x1_n+a*y1_n)
    z1_n1=z1_n+h*(b+z1_n*(x1_n-c))   

    x2_n1=x2_n+h*(-w2*y2_n-z2_n)
...
peakslistx1, _ = find_peaks(x1,  distance= 1)

peakslistx2=[] #list containing the indexes of each trajectory's peaks
peakslistx2, _ = find_peaks(x2,  distance= 1)
...
for i in range(len(tx2_values)):
    if i+1<len(tx2_values):
        dif=-tx2_values[i]+tx2_values[i+1]
        periodo2.append(dif)
print(numpy.mean(periodo2))
